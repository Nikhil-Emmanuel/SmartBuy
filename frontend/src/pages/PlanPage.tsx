    ...
    <div className="mx-auto max-w-4xl space-y-6 px-4 pb-16 pt-8 sm:px-6">
      ...
      <Card>
        <CardContent className="grid grid-cols-2 gap-4 p-5 sm:grid-cols-4">
          <Stat label="Budget" value={rupees(data.totals.budget)} />
          <Stat label="This bundle" value={rupees(bundle.total_cost)} danger={overBudget} />
          <Stat label="You save" value={rupees(bundle.total_savings)} accent="savings" />
          <Stat
            label={overBudget ? "Over by" : "Remaining"}
            value={rupees(overBudget ? bundle.over_budget : bundle.remaining_budget)}
            danger={overBudget}
          />
        </CardContent>
      </Card>
      ...
      <div className="space-y-2.5">
        {bundle.items.map((item) => (
          <BundleItemRow
            ...
            onExplain={() => setExplainTarget({ product: item.product, requirement: item.requirement })}
          />
        ))}
      </div>
    ...
    <div>
      ...
      <p
        className={
          ...
          (danger ? "text-danger" : accent === "savings" ? "text-savings" : "text-foreground")
        }
      >
        {value}
      </p>
