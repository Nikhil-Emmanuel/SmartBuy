...
- Collaborative filtering is implemented but **off by default** (`ENABLE_COLLABORATIVE_FILTERING`).
  ...
  runs `train.py`, and nothing learns from live traffic.
- The shopper-segment classifier that issues personalised coupons is trained on **synthetic**
  behaviour generated by `backend/scripts/seed.py`. Its 96.8% holdout accuracy shows the pipeline
  recovers structure we put into the data — it is **not** evidence about how real shoppers behave,
  and must not be quoted as if it were. It refuses to predict on thin history or low confidence
  rather than guessing. See [PERSONALIZATION.md](docs/PERSONALIZATION.md).
